...
export const UserProvider = ({ children }: Props) => {
  ...
  const fetchUserInfo = useCallback(async () => {
    ...
    if (token) {
      try {
        const res = await axios.get("https://community-git-main-8ird8s-projects.vercel.app/api/userId", {
  ...
  const fetchCurrentUser = useCallback(async () => {
    if (userInfo.userId) {
      try {
        const res = await axios.get(`https://community-git-main-8ird8s-projects.vercel.app/api/userInfo/${userInfo.userId}`, {
          withCredentials: true,
        });
